...
function test(states) {
  ...
  if (allStatesVisited()) {
    ...
    document.getElementById("log").innerHTML += `<br><strong>All states have been visited it took ${total_time} seg and the vacuum visited ${state_visited} states before the vacuum stop!</strong>`;
  ...
  // save the previous state of A and B it will be used to inform about the changes
  var previousA = states[1];
  var previousB = states[2];

  // pick a random state extern agent
  var randomState = Math.floor(Math.random() * 8);

  // Aplicar cambio aleatorio de agente externo
  exterAgent(randomState, previousA, previousB);
